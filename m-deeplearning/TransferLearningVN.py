...
tl_model_file = os.path.join(base_folder, "Output", "TransferLearning_recognitionP.model")
...
max_epochs = 30
mb_size = 100
lr_per_mb = [0.1]*5 + [0.01]*10 + [0.005]*10 + [0.001]*5
momentum_per_mb = 0.9
...
def create_mb_source(map_file, image_width, image_height, num_channels, num_classes, randomize=True,is_training=True):
    ...
    if is_training:
        ...
        transforms += [
            xforms.color(brightness_radius=0.5, contrast_radius=0.5, saturation_radius=0.5),
        ]
...
def train_model(base_model_file, feature_node_name, last_hidden_node_name,
                ...
                num_epochs,learningRateMB=lr_per_mb, max_images=-1, freeze=False):
    ...
    lr_schedule = learning_rate_schedule(learningRateMB, unit=UnitType.minibatch)
    ...
    for epoch in range(num_epochs):       # loop over epochs
        ...
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            ...
            if sample_count % (300 * mb_size) == 0:
                print ("Processed {0} samples".format(sample_count))
        tl_model.save(os.path.join(os.path.dirname(os.path.abspath(__file__)),"Output","ResNet18_CNTK_transferedVN_{0}.model".format(epoch)))
